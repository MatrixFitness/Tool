#!/bin/sh
# Matrix Cloud Bootstrap - Build 061
...
DOCTOR_BUILD="$(sed -n 's/^MATRIX_DOCTOR_BUILD=//p' /usr/local/bin/matrix-doctor 2>/dev/null | head -1 | tr -d '\r\n ')"
if [ -z "$DOCTOR_BUILD" ]; then
    DOCTOR_BUILD="$(grep -m1 '^# Matrix Doctor - Build [0-9][0-9][0-9]' /usr/local/bin/matrix-doctor 2>/dev/null | sed 's/.*Build //;s/[^0-9].*//' | head -1)"
fi
[ "$DOCTOR_BUILD" = "$BUILD_PAD" ] || fail "Matrix Doctor build marker is ${DOCTOR_BUILD:-missing}, expected $BUILD_PAD"
