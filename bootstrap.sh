...
if ! opkg list-installed 2>/dev/null | grep -q '^zerotier '; then
    log "Installing missing ZeroTier package..."
    opkg install zerotier >>"$LOG" 2>&1 || fail "Could not install ZeroTier package"
fi
...
# Build 027: install critical tools before recovery
...
# Build 027 unified ZeroTier runtime library
...
# Build 027 - install self-healing Doctor before recovery validation.
...
log "
# Build 027 Admin Authentication Recovery
