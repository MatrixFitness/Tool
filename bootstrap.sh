#!/bin/sh
# Matrix Cloud Bootstrap - Build 073
...
# Build073: mandatory second-stage network enforcement.
# The freshly downloaded package is still available in /tmp/MatrixInstall, so
# this runs even if a previous updater path skipped a post-install hook.
log "[7b/9] Enforcing Matrix standard network policy..."
NETWORK_STANDARD="/tmp/MatrixInstall/bin/matrix-network-standard"
[ -x "$NETWORK_STANDARD" ] || NETWORK_STANDARD="/usr/local/bin/matrix-network-standard"
[ -x "$NETWORK_STANDARD" ] || fail "matrix-network-standard is missing"
"$NETWORK_STANDARD" >>"$LOG" 2>&1 || fail "Matrix standard network policy failed"

SSID24="$(uci -q get wireless.default_radio0.ssid)"
SSID5="$(uci -q get wireless.default_radio1.ssid)"
MOBILE_METRIC="$(uci -q get network.mob1s1a1.metric)"
WAN_METRIC="$(uci -q get network.wan.metric)"
[ "$SSID24" = "egym" ] || fail "2.4 GHz standard not applied: $SSID24"
[ -n "$SSID5" ] || fail "5 GHz SSID is empty"
[ "$MOBILE_METRIC" = "1" ] || fail "Mobile metric is $MOBILE_METRIC, expected 1"
[ "$WAN_METRIC" = "10" ] || fail "WAN metric is $WAN_METRIC, expected 10"
