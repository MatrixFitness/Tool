...
if /usr/local/bin/matrix-doctor | grep -q "SYSTEM HEALTH: REPAIR ADVISED"; then
  log "[8/9] Repair advised, running recovery again..."
  MATRIX_KEEP_TMP_INSTALL=1 ZT_NETWORK_ID="$ZT_NETWORK_ID" /usr/local/bin/matrix-recovery
  /usr/local/bin/matrix-doctor | tee -a "$LOG"
fi
log "[9/9] Finalizing..."
...
if [ -n "$ZT_IP" ]; then
 log "ZeroTier Toolkit: http://$ZT_IP:8080"; log "ZeroTier Admin:   http://$ZT_IP:8081/cgi-bin/admin_login.sh"
else
 log "ZeroTier remote access is not ready yet."; [ -n "$ZT_NODE" ] && log "Authorize Node ID in ZeroTier Central: $ZT_NODE"
fi
